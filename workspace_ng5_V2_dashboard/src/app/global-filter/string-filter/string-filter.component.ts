...
export class StringFilterComponent implements OnInit, AfterViewInit, OnDestroy {

    // static componentId = LazyComponentConfiguration.StringFilter.componentName;
    filterTabList: any;
    StringFilterObj: any;
    FilterByConfig: any;
    ...
    ngOnInit() {
        ...
        if (!(this.StringFilterObj.appliedFilter) && !(this.StringFilterObj.filterCdnSet || this.StringFilterObj.filterValueSet)) {
            this.StringFilterObj.FilterConditionOperator.op = DashboardConstants.ReportObjectOperators.In;
            this.StringFilterObj.FilterConditionOperator.title = DashboardConstants.ReportObjectOperators[DashboardConstants.ReportObjectOperators.In];
    ...
    onChangeFilterByOp(FilterObj, op) {
        ...
        if(FilterObj != undefined && (FilterObj.FilterConditionOperator.op == 7 || FilterObj.FilterConditionOperator.op == 6 ) && FilterObj.FilterConditionText != ''){ 
            var tempChipArray = [];
    ...
    setFilterListInSelected() {
        if (this.StringFilterObj != undefined) {
            ...
            if (
                ...
                this._loaderService.showGlobalLoader();
                if (this.StringFilterObj.FilterType === DashboardConstants.FilterType.MultiSelect) {
                    this.getFilterData(_daxReportFilterObject)
                        .toPromise()
                        .then((_response: Array<IFilterList>) => {
                            if (this.StringFilterObj.FilterList.length == 0) {
    ...
    private configureFilterByCondition(_reportObject: any) {
        ...
        switch (_reportObject.FilterType) {
            case DashboardConstants.FilterType.MultiSelect:
                this.filterByCdnConfig.options = map(lstOfConditions, (obj, index) => {
